...
TurbulenceFields::TurbulenceFields(Real nu, Real dt, Real tau, int imax, int jmax, Real UI, Real VI, Real PI, Real TI, Real KI, Real EPSI, Real alpha,
               Real beta, Real gx, Real gy) : Fields(nu, dt, tau, imax, jmax, UI, VI, PI, TI, KI, EPSI, alpha,
               beta, gx, gy) {}

void TurbulenceFields::calculate_nu_t(Grid &grid) {
    for (const auto &current_cell : grid.fluid_cells()) {
        int i = current_cell->i();
        int j = current_cell->j();
...
void TurbulenceFields::calculate_k_and_epsilon(Grid &grid) {
    auto K_OLD = _K;
    auto EPS_OLD = _EPS;
...
Real TurbulenceFields::damp_f2(int i, int j) {
    return 1 - 0.3 * std::exp(- std::pow(k(i, j) * k(i, j) / (_nu * eps(i, j)), 2));
}

Real TurbulenceFields::damp_fnu(int i, int j) {
    return std::exp(- 3.4 / (std::pow(k(i, j) * k(i, j) / (_nu * eps(i, j) * 50), 2)));
}

void TurbulenceFields::calculate_fluxes(Grid &grid, bool calc_temp) {
    for (const auto &current_cell : grid.fluid_cells()) {
        int i = current_cell->i();
        int j = current_cell->j();
        f(i, j) = u(i, j) + dt() * (( Discretization::interpolate(nu_t_matrix(), i, j, 1, 0)) * 
                                    Discretization::diffusion(u_matrix(), i, j) -
                                    Discretization::convection_u(u_matrix(), v_matrix(), i, j));
        g(i, j) = v(i, j) + dt() * (( Discretization::interpolate(nu_t_matrix(), i, j, 0, 1)) * 
...
void TurbulenceFields::calculate_temperatures(Grid &grid) {
    auto T_old = _T;
    for (const auto &current_cell : grid.fluid_cells()) {
        int i = current_cell->i();
        int j = current_cell->j();
...
Real TurbulenceFields::calculate_dt(Grid &grid, bool calc_temp) {
