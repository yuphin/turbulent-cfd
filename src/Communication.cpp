...
void Communication::communicate(Params *params, Matrix<Real> &matrix) {
    ...
    MPI_Type_vector(block_count_col, block_length_col, stride_col, VORTIGEN_MPI_REAL, &column_type);
    ...
    int stride_row = 1;
    // TODO: Might need to use typedef later to adapt to different Real datatype.
    MPI_Type_vector(block_count_row, block_length_row, stride_row, VORTIGEN_MPI_REAL, &row_type);
...
Real Communication::reduce_all(Real loc_value, MPI_Op mpi_operation) {
    ...
    // TODO: Might need to use typedef later to adapt to different Real datatype.
    MPI_Allreduce(&loc_value, &global_value, 1, VORTIGEN_MPI_REAL, mpi_operation, MPI_COMM_WORLD);
    return global_value;
}
