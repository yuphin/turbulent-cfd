...
double Discretization::convection_u(const Matrix<double> &U, const Matrix<double> &V, int i, int j) {
    ...
    double result_fd = (((U(i, j) + U(i + 1, j)) / 2) * ((U(i, j) + U(i + 1, j)) / 2) 
                        - ((U(i - 1, j) + U(i, j)) / 2) * ((U(i - 1, j) + U(i, j)) / 2)) / _dx;
    double result_dc = _gamma * (std::abs(U(i, j) + U(i + 1, j)) / 2 * (U(i, j) - U(i + 1, j)) / 2
                        - std::abs(U(i - 1, j) + U(i, j)) / 2 * (U(i - 1, j) - U(i, j)) / 2) / _dx;
    result += result_fd + result_dc;

    // dUV/dy
    result_fd = ((V(i, j) + V(i + 1, j)) / 2 * (U(i, j) + U(i, j + 1)) / 2
                        - (V(i, j - 1) + V(i + 1, j - 1)) / 2 * (U(i, j - 1) + U(i, j)) / 2) / _dy;
    result_dc = _gamma * (std::abs(V(i, j) + V(i + 1, j)) / 2 * (U(i, j) - U(i, j + 1)) / 2
                        - std::abs(V(i, j - 1) + V(i + 1, j - 1)) / 2 * (U(i, j - 1) - U(i, j)) / 2) / _dy;
    result += result_fd + result_dc;
...
double Discretization::convection_v(const Matrix<double> &U, const Matrix<double> &V, int i, int j) { 
    ...
    double result_fd = (((V(i, j) + V(i, j + 1)) / 2) * ((V(i, j) + V(i, j + 1)) / 2) 
                        - ((V(i, j - 1) + V(i, j)) / 2) * ((V(i, j - 1) + V(i, j)) / 2)) / _dy;
    double result_dc = _gamma * (std::abs(V(i, j) + V(i, j + 1)) / 2 * (V(i, j) - V(i, j + 1)) / 2
                        - std::abs(V(i, j - 1) + V(i, j)) / 2 * (V(i, j - 1) - V(i, j)) / 2) / _dy;
    result += result_fd + result_dc;

    // dUV/dx
    result_fd = ((U(i, j) + U(i, j + 1)) / 2 * (V(i, j) + V(i + 1, j)) / 2
                        - (U(i - 1, j) + U(i - 1, j + 1)) / 2 * (V(i - 1, j) + V(i, j)) / 2) / _dx;
    result_dc = _gamma * (std::abs(U(i, j) + U(i, j + 1)) / 2 * (V(i, j) - V(i + 1, j)) / 2
                        - std::abs(U(i - 1, j) + U(i - 1, j + 1)) / 2 * (V(i - 1, j) - V(i, j)) / 2) / _dx;
    result += result_fd + result_dc;
